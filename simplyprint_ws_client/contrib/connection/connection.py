...
class Connection(Generic[T]):
    ...
    def deliver(self, event: ConnectionEvent) -> None:
    ...
    async def _emit_event(self, event: ConnectionEvent) -> None:
        if not self.closed:
            await self.event_bus.emit(event)

    @staticmethod
    def _lossless_event(event: ConnectionEvent) -> bool:
        return not isinstance(event, MessageReceived) or event.lossless


class MqttConnection(Connection[MqttTransport]):
...
class WsConnection(Connection[WsTransport]):
