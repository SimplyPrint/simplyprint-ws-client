...
:class:`~simplyprint_ws_client.contrib.connection.transport.Transport` per endpoint key,
ref-counted, and hands every caller a :class:`~simplyprint_ws_client.contrib.connection.connection.Connection`
...
class Endpoint(Generic[T]):
    ...
    async def fan_lifecycle(self, event: ConnectionEvent) -> None:
...
class Pool(Generic[T]):
    ...
    def __init__(
        ...
        *,
        ...
        self.lease_class = lease_class or Connection
    ...
    def connect(self, url: object, params: object = None) -> "Connection[T]":
    ...
    def release(self, lease: "Connection[T]") -> Optional[T]:
