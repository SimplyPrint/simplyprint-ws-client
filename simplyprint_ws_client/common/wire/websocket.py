...
A printer client reaches a WebSocket endpoint through one call::
    ...
    from simplyprint_ws_client.common.wire import ws
...
the wire. A per-``impl``
:class:`~simplyprint_ws_client.common.wire.pool.Pool` keyed by the full URL
shares one socket across every lease on the same endpoint.

Everything beyond ``url``/``impl``/``pool`` is carried by one
:class:`~simplyprint_ws_client.common.wire.options.ConnectionOptions`.
"""
...
from simplyprint_ws_client.common.wire.messages import WsKind, WsMessage
...
from simplyprint_ws_client.common.wire.options import (
    ConnectionOptions,
    WireKeepalive,
)
from simplyprint_ws_client.common.wire.pool import Pool
from simplyprint_ws_client.common.wire.pools import DefaultPools
...
def connect(
    ...
    *,
    ...
    once; readiness is awaited via :meth:`~simplyprint_ws_client.common.wire.lease.Lease.ready`.
