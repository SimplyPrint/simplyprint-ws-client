...
:class:`~simplyprint_ws_client.common.wire.reconnect.Reconnecting` supervisor.
...
:class:`~simplyprint_ws_client.common.wire.transport.Transport` consumer (the pool
...
class Paho(MqttTransport):
    ...
    def transition(self, state: ConnectionState, event: WireEvent) -> None:
        self.state = state
        self.post(event)
    ...
    def post(self, event: WireEvent) -> None:
