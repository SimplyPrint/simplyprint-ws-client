...
:class:`Websockets` is a 1:1 WebSocket :class:`~simplyprint_ws_client.wire.transport.WsTransport`
built on the supervised :class:`~simplyprint_ws_client.wire.reconnect.Reconnecting`
...
:class:`~simplyprint_ws_client.wire.transport.TransientError`, which ends
...
class Websockets(WsTransport, Reconnecting):
    ...
    :class:`~simplyprint_ws_client.wire.policy.RetryPolicy`, the
    ...
    async def recv(self) -> WsMessage:
        ...
        :class:`~simplyprint_ws_client.wire.transport.TransientError` so the
