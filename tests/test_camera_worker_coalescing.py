...
async def test_start_pause_storm_collapses_to_one_worker():
    ...
    def hammer():
        for i in range(25):
            (handle.start if i % 2 == 0 else handle.pause)()

    thread = threading.Thread(target=hammer)  # some commands from a foreign thread
    thread.start()
    hammer()
    thread.join()
    handle.start()  # final desired state: RUNNING
    await asyncio.sleep(0.2)  # let the reconcile settle

    try:
        assert await asyncio.wait_for(handle.receive_frame(), timeout=1) == b"f"
...
async def test_oneshot_poll_burst_is_bounded():
    ...
    receivers = [asyncio.create_task(handle.receive_frame()) for _ in range(20)]
    frames = await asyncio.wait_for(asyncio.gather(*receivers), timeout=1)

    try:
        assert frames == [b"snap"] * 20
...
async def test_frames_stop_after_pause_settles():
    ...
    count = {"n": 0}
    original_deliver = handle.deliver_frame

    def counting_deliver(data, ts):
        count["n"] += 1
        original_deliver(data, ts)

    handle.deliver_frame = counting_deliver
